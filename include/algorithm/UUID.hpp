...
namespace HORIZON::ALGORITHM
{
	...
	{
	private:
		static constexpr const size_t UUID_LENGTH     = 8 + 1 + 4 + 1 + 4 + 1 + 4 + 1 + 12;
	...
	public:
		...
		{
			for (size_t i = 0; i < UUID_LENGTH_RAW; ++i)
			{
				if (_uuid[i] != other._uuid[i]) { return false; }
			}
			return true;
		}

		constexpr bool operator==(UUID const& other) const noexcept { return Equals(other); }

		explicit operator std::string() const noexcept { return std::string(_uuidString); }
	};
