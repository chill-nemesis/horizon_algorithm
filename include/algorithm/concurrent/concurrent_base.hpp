...
namespace HORIZON::ALGORITHM::CONCURRENT
{
     ...
     * @details This class adds concurrent access utility and lock functionality. It acts as an interface to prevent accidental race conditions
     * while working as a standard STL container. As with everything C/C++-related, this cannot guarantee or prevent malicious behaviour. However,
     * this must be a conscious choice of the implementer to circumvent the safety-mechanics of the containers.
     *
     * You will find that all methods provide two signatures: A default (stl-like) implementation and one requiring an access_token. Access_tokens
     * are supposed to mark ownership of the container. This is useful if you need to make multiple calls to the container without interference of
     * other threads. An access_token can be obtained by using the Guard()-methods of the container. Once a token is out of scope (or manually
     * unlocked) the ownership is released. Each "default" method signature will acquire a guard token and then call the guarded implementation.
     */
    ...
    protected:
        ...
        // see https://stackoverflow.com/questions/27726818/stdcondition-variablewait-for-exits-immediately-when-given-stdchronodura
        // (second answer) for the gcc work-around
        // technically, this is expected behaviour. GCC and windows are just nice to the user and do not create an overflow
        // the standard explicitly warn about this behaviour: "Note that rel_time must be small enough not to overflow when added to std::chrono::steady_clock::now()."
    ...
    public:
         ...
         *  @endcode
         */
        friend void swap(concurrent_base& left, concurrent_base& right, access_token const& leftLock, access_token const& rightLock) noexcept
        {
        ...
        /*!
         * @brief Default swap operation for concurrent_base is not supported. Use @sa swap(concurrent_base&, concurrent_base&, access_token
         * const&, access_token const&) instead.
         * @relates concurrent_base
         * @param left
         * @param right
         */
